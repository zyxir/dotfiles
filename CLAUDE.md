...
- **VSCodium**: Regenerate `extensions.txt` with `codium --list-extensions > per_app/vscodium/extensions.txt` after installing or removing extensions. Extensions in `_DISABLED_EXTENSIONS` (in `install.py`) are installed but disabled globally by default — enable per-project from the Extensions view when needed.
- **scripts/test-doh.py**: Benchmarks DoH servers for reachability and latency using RFC 8484 wire format over HTTP/2 (via curl). Tests every server in both DIRECT and PROXY modes in a single table — two domains × two modes = four columns per row. Re-run when changing DNS providers or if connectivity issues arise.
- **subconv (wisp)**: Subscription converter under `per_host/wisp/subconv/`. `subconv.py` reuses `Script.js` directly — reads it at runtime, appends a stdin/stdout CLI wrapper, runs it via `node`. Python handles YAML/HTTP; Script.js handles transform logic. Reads `subconv/subconv.env` for the subscription URL (separate from `.env` — changes more often). Outputs to `subconv/srv/<secret>/ZyProxy`, served as a static file by Caddy at `subconv.<domain>/<secret>/ZyProxy`. Run via cron every 60 min (provider rate limits). Depends on `nodejs` and `python3-yaml` (installed by `bootstrap/vps/vps_bootstrap.sh`). `Script.js` can also be copied manually into Clash Verge Rev as a backup when the VPS is unavailable.
  - **refresh_hosts (wisp)**: Lightweight tailnet hosts updater under `per_host/wisp/subconv/refresh_hosts.py`. Runs `tailscale status --json`, builds a `hosts:` mapping of peer DNS names → Tailscale IPs, and updates the existing ZyProxy YAML in-place. No external requests — safe to run every 1 min via cron (job name: `refresh_host`). Uses only `python3-yaml`.
